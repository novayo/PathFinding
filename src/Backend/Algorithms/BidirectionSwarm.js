...
function BidirecionSwarm(startCallback, speed) {
    var retSearchPath = [];
    var retBombPath = [];
    var retShortestPath = [];

    if (position.bomb) {
        retShortestPath = retShortestPath.concat(DoBidirecionSwarm(position.start, position.bomb, retSearchPath))

        // 有找到最小路徑才繼續
        if (retShortestPath.length > 0) {
            retShortestPath = retShortestPath.concat(DoBidirecionSwarm(position.bomb, position.end, retBombPath))
        }
    } else {
        retShortestPath = retShortestPath.concat(DoBidirecionSwarm(position.start, position.end, retSearchPath))
    }
    // 執行 start 動畫
...
function DoBidirecionSwarm(startPos, endPos, searchPath) {
    // 先假設權重都是1
    var i, j;
    var weights = {}
...
export default BidirecionSwarm;