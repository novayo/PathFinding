...
import { CreateAround, Random, RandomEven } from './Core';
...
function DoRecursiveDivision(x, y, width, height, walls, boolDirection = null) {
    ...
    if (curDirection === direction.horizontal) {
        ...
        DoRecursiveDivision(x, y, width, randomX - x, walls, boolDirection); // 第一個規定跑水平

        // 跑 新x, y, width 且 height變成了 目前總高 - 剛剛選的高度(跑下面)
        DoRecursiveDivision(randomX, y, width, height - (randomX - x), walls, !boolDirection); // 第二個規定跑垂直
    } else {
        // 垂直雷同
        ...
        DoRecursiveDivision(x, y, randomY - y, height, walls, boolDirection); // 第一個規定跑水平
        DoRecursiveDivision(x, randomY, width - (randomY - y), height, walls, !boolDirection); // 第二個規定跑垂直
    }
}
